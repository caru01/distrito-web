...
export default function OrderTracker({ open, onClose, initialOrder }) {
  ...
  const statusMeta = orderStatusMeta(status, { deliveryType: order?.delivery_type, hasDriver: Boolean(order?.driver?.name) });
  ...
  return <div className="tracker-overlay" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="tracker-panel" aria-modal="true" role="dialog"><header><div><span>Seguimiento en tiempo real</span><h2>¿Dónde está mi pedido?</h2></div><button onClick={onClose} aria-label="Cerrar"><X /></button></header>
    ...
    {order && <div className="tracker-result"><div className="tracker-order-heading"><div><span>Pedido #{order.id}</span><h3>{order.customer_name}</h3></div><div className="tracker-status-group"><strong className={`tracker-status ${status === 'Cancelado' ? 'cancelled' : ''}`} style={{ color: statusMeta.color, background: statusMeta.background, borderColor: `${statusMeta.color}66` }}>{displayedStatus}</strong>{status === 'Entregado' && deliveredDuration && <small>Entregado en {deliveredDuration}</small>}</div></div>
      {order.delivery_status === 'En camino' && !isExternalDelivery && hasDriverLocation && <section className="tracker-live-map"><div className="tracker-driver-heading"><div><span className="tracker-live-dot"/><div><b>{order.driver?.name || 'Tu domiciliario'} va en camino</b><small>{[order.driver?.vehicle_type, order.driver?.plate].filter(Boolean).join(' · ') || 'Ubicación en vivo'}</small></div></div><a href={`https://www.google.com/maps/search/?api=1&query=${order.driver.latitude},${order.driver.longitude}`} target="_blank" rel="noreferrer">Abrir mapa</a></div><LiveDeliveryMap apiKey={import.meta.env.VITE_GOOGLE_MAPS_API_KEY || ''} mapId={import.meta.env.VITE_GOOGLE_MAPS_MAP_ID || 'DEMO_MAP_ID'} store={order.store} destinations={order.destination ? [order.destination] : []} drivers={liveDrivers} trail={order.driver?.trail || []} selectedDriverId={liveDrivers[0]?.id || null} showJourney ariaLabel="Recorrido en vivo del domiciliario"/><small>Última ubicación: {new Date(order.driver.updated_at).toLocaleTimeString('es-CO', { hour: 'numeric', minute: '2-digit', timeZone: 'America/Bogota' })}</small></section>}
      {isExternalDelivery && <section className="tracker-external-delivery"><Truck/><div><b>Tu pedido está siendo entregado por un operador logístico aliado.</b><small>{order.external_delivery?.company_name ? `Operador: ${order.external_delivery.company_name}. ` : ''}{order.external_delivery?.eta_minutes ? `Tiempo estimado: ${order.external_delivery.eta_minutes} minutos.` : 'Sigue aquí los estados confirmados.'}</small><em>No mostramos un mapa porque este operador no comparte GPS con Distrito BG.</em></div></section>}
      {order.delivery_status === 'En camino' && !isExternalDelivery && !hasDriverLocation && <section className="tracker-external-delivery tracker-gps-waiting"><MapPin/><div><b>Tu domiciliario va en camino.</b><small>El mapa aparecerá cuando recibamos una señal GPS real.</small></div></section>}
      {status === 'Cancelado' ? <div className="tracker-cancelled">Este pedido fue cancelado. Comunícate con el restaurante si necesitas ayuda.</div> : <div className="tracker-timeline">{steps.map((step, index) => <div className={`tracker-step ${index <= currentIndex ? 'done' : ''} ${index === currentIndex ? 'current' : ''}`} key={step}><span>{index < currentIndex ? <CheckCircle /> : step === 'En camino' ? <Truck /> : step === 'Entregado' ? <MapPin /> : step === 'Listo' ? <PackageCheck /> : <Clock />}</span><small>{orderStatusLabel(step, order.delivery_type)}</small></div>)}</div>}
      <div className="tracker-summary"><div><span>Actualizado</span><strong>{new Date(order.updated_at || order.created_at).toLocaleString('es-CO', { dateStyle: 'medium', timeStyle: 'short', timeZone: 'America/Bogota' })}</strong></div><div><span>Total</span><strong>${Number(order.total).toLocaleString('es-CO')}</strong></div></div>
